  ...
  const createOrder = async (productId, quantity) => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/orders", {
        ...
        body: JSON.stringify({ user_id: user.id, product_id: productId, quantity, email: user.email }),
      });
      if (!response.ok) {
        const errorText = await response.text();
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              <InfoCard title="Active Orders" value={orders.length} icon={Building} color="green" unit="" />
              <InfoCard title="CO₂ Saved" value="2.4" unit="tons" icon={Leaf} color="emerald" />
              <InfoCard title="Water Saved" value="1.2" unit="kL" icon={Droplets} color="green" />
            </div>
            <div className="bg-black/80 p-6 rounded-xl border border-green-400/30 shadow-lg shadow-green-400/20 hover:shadow-green-400/30 transition-all duration-300">
              <h2 className="text-xl font-bold mb-4 text-green-400">Nearby Suppliers</h2>
              <div className="bg-black/60 h-64 rounded-xl flex items-center justify-center text-green-300 relative overflow-hidden">
                ...
                <div className="text-center relative z-10">
                  ...
                  <p className="text-sm text-green-300/70">Real-time supplier locations</p>
                </div>
              </div>
            </div>
          </div>
        ...
        return (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <h2 className="text-xl font-bold text-green-400">Find Suppliers</h2>
            ...
            <div className="grid gap-4">
              {products.length === 0 ? (
                <div className="text-center text-green-300/70 p-6">No products found.</div>
              ) : (
                products.map((product) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-white gap-4">
                      <div>
                        <h3 className="font-semibold text-lg text-green-400">{product.name}</h3>
                        <div className="text-sm text-green-300/70 mt-2 space-x-6">
                          <span>Available: <strong>{product.quantity_available} tons</strong></span>
                          <span>Price: <strong>₹{product.price}/ton</strong></span>
                        </div>
                      </div>
                      <button
                        onClick={() => createOrder(product.id, 1)} // Example quantity
      ...
      case "calculator":
        return (
          <div className="bg-black/80 p-6 rounded-xl border border-green-400/30 text-white shadow-lg shadow-green-400/20 hover:shadow-green-400/30 transition-all duration-300">
            <h2 className="text-xl font-bold mb-6 text-green-400">CO₂ & Water Savings Calculator</h2>
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <label htmlFor="bricks" className="block text-sm mb-2 text-green-300/70">Number of Bricks</label>
              ...
              <div className="space-y-6">
                <div className="bg-green-900/30 p-6 rounded-xl border border-green-400/30 shadow-lg shadow-green-400/20 hover:shadow-green-400/30 transition-all duration-300">
                  <h3 className="text-lg font-semibold text-green-200 mb-2">CO₂ Savings</h3>
                  <p className="text-3xl font-bold text-green-400">{Number(calculatorData.co2Savings).toLocaleString()} kg</p>
                  <p className="text-sm text-green-300/70 mt-1">≈ {Math.round(calculatorData.co2Savings / 22)} trees planted</p>
                </div>
                <div className="bg-green-900/30 p-6 rounded-xl border border-green-400/30 shadow-lg shadow-green-400/20 hover:shadow-green-400/30 transition-all duration-300">
                  <h3 className="text-lg font-semibold text-green-200 mb-2">Water Savings</h3>
                  <p className="text-3xl font-bold text-green-400">{Number(calculatorData.waterSavings).toLocaleString()} L</p>
                  <p className="text-sm text-green-300/70 mt-1">≈ {Math.round(calculatorData.waterSavings / 150)} people/day</p>
                </div>
              </div>
            </div>
          </div>
      ...
      case "my-orders":
        return (
          <div className="space-y-6">
            ...
            {orders.length === 0 ? (
              <div className="text-center text-green-300/70 p-6">No orders found.</div>
            ) : (
              <div className="grid gap-4">
                {orders.map((order) => (
                  <div key={order.id} className="bg-black/80 p-6 rounded-xl border border-green-400/30 shadow-lg shadow-green-400/20">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="font-semibold text-green-400">Order #{order.id}</p>
                        <p className="text-sm text-green-300/70">Quantity: {order.quantity} tons</p>
                        <p className="text-sm text-green-300/70">Total: ₹{order.total_price.toLocaleString()}</p>
                      </div>
                      <p className="text-xs text-green-300/50">{new Date(order.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      case "messages":
        return (
          <div className="space-y-4">
            {messages.length === 0 ? (
              <div className="text-center text-green-300/70 p-6">No messages available.</div>
            ) : (
              messages.map((message) => (
                <div key={message.id} className="bg-black/80 p-4 rounded-xl border border-green-400/30">
                  <div className="flex justify-between items-center">
                    <div>
          ...
          <ul className="space-y-2">
            {[
              ...
              <li key={id}>
                ...
                >
                  <Icon
                    className={`w-5 h-5 ${activeTab === id ? "text-black" : "text-green-400"} group-hover:scale-110 transition-transform duration-300`}
                  />
        ...
        <div className="p-4 border-t border-green-400/30">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-green-300/70">{profile?.role || "Buyer"}</p>
            </div>
          </div>
          <button
            onClick={signOut}
      ...
      <div className="flex-1 p-6">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-green-300/70">
            Welcome back, {user?.user_metadata?.name || profile?.name || user?.email?.split("@")[0] || "User"}!
          </p>
          {renderContent()}
        </div>
      </div>
